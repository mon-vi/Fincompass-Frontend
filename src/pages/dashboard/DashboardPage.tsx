  ...
  return (
    <div className="space-y-8">
      <SectionHeader
        ...
        subtitle="Your dashboard highlights where you stand, what needs attention, and the next move to make."
      />
      ...
      <section className="overflow-hidden rounded-[2rem] bg-[#12355b] text-white shadow-2xl shadow-slate-900/10">
        <div className="grid gap-6 p-6 lg:grid-cols-[1.15fr_0.85fr] lg:p-8">
          <div>
            <div className="flex flex-wrap items-center gap-2">
              ...
              {dashboard.notificationCount > 0 && <Badge variant="warning">{dashboard.notificationCount} notification{dashboard.notificationCount === 1 ? '' : 's'}</Badge>}
            </div>
            ...
            <p className="mt-3 max-w-2xl text-sm leading-6 text-sky-100">
              Start with cash flow, watch due-soon items, then follow the action plan. FinCompass keeps the signal above the noise.
            </p>
          ...
          <div className="rounded-3xl bg-white/10 p-5 ring-1 ring-white/15">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-2 text-2xl font-black tracking-tight">
                  {isLoading ? 'Loading your next move' : dashboard.actionPlan.nextActionTitle ? `Next: ${dashboard.actionPlan.nextActionTitle}` : dashboard.dueSoon[0] ? `Pay ${dashboard.dueSoon[0].name}` : 'Review your budget'}
                </p>
              </div>
            ...
            <Link to={ROUTES.ACTION_PLAN} className="mt-5 inline-flex min-h-11 items-center justify-center rounded-xl bg-white px-4 py-2 text-sm font-bold text-[#12355b] transition hover:bg-sky-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-2 focus-visible:ring-offset-[#12355b]">
              Open action plan
            </Link>
          </div>
        </div>
      ...
      {data?.dueSoon && data.dueSoon.length > 0 && (
        <Alert variant="warning" title="Payment attention">
          Your next minimum payment of {formatCurrency(data.dueSoon[0].minimumPayment)} is due in {data.dueSoon[0].daysUntilDue} day{data.dueSoon[0].daysUntilDue !== 1 ? 's' : ''}. Check the due-soon card below for details.
        </Alert>
      )}
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-3">
                <div className="h-2 flex-1 overflow-hidden rounded-full bg-slate-100 ring-1 ring-inset ring-slate-200">
                  <div
                    className="h-full rounded-full bg-[#2b6d91] transition-all"
              ...
              {dashboard.actionPlan.nextActionTitle ? (
                  ...
                  <p className="rounded-2xl bg-slate-50 p-4 text-sm text-slate-500">No action plan items yet. Your next steps will appear here once your profile has enough detail.</p>
                )}
              </div>
          )}
        </Card>
      ...
      <div
        className={cn(
          'rounded-[2rem] border p-6 text-center shadow-sm shadow-slate-900/[0.04]',
      ...
      >
        {user?.tier === 'cfo' ? (
          <>
            ...
            <Link to={ROUTES.ARIA} className="mt-4 inline-flex min-h-10 items-center justify-center rounded-xl bg-[#12355b] px-4 py-2 text-xs font-bold text-white hover:bg-[#0b2746]">Open ARIA</Link>
          </>
        ) : (
          <>
            <p className="text-sm font-bold text-slate-700">ARIA is available with the CFO plan</p>
            <p className="mt-1 text-xs text-slate-500">A natural upgrade when you want personalized AI-powered financial guidance.</p>
            <Link to={ROUTES.BILLING} className="mt-4 inline-flex min-h-10 items-center justify-center rounded-xl border border-slate-300 bg-white px-4 py-2 text-xs font-bold text-slate-700 hover:bg-slate-50">See upgrade options</Link>
          </>
        )}
      </div>
    </div>
