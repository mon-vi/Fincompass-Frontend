    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        <StatCard
          ...
          value={isLoading ? '—' : formatCurrency(data!.financialSummary.monthlyIncome)}
          isLoading={isLoading}
        ...
        <StatCard
          ...
          value={isLoading ? '—' : formatCurrency(data!.financialSummary.monthlyExpenses)}
          isLoading={isLoading}
        />
        <StatCard
          label="Debt payments"
          value={isLoading ? '—' : formatCurrency(data!.financialSummary.monthlyDebtPayments)}
          isLoading={isLoading}
        />
        <StatCard
          label="Net cash flow"
          value={isLoading ? '—' : formatCurrency(data!.financialSummary.netCashFlow)}
          subtext={data && data.financialSummary.netCashFlow > 0 ? 'Positive' : 'Negative'}
          trend={data && data.financialSummary.netCashFlow > 0 ? 'up' : 'down'}
          isLoading={isLoading}
        />
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <Card className="flex flex-col items-center">
          ...
          {isLoading ? (
            ...
            <ScoreGauge score={data!.healthScore.score} grade={data!.healthScore.grade} trend={data!.healthScore.trend} />
          )}
        </Card>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-semibold text-slate-900">
                  {formatCurrency(data!.budgetSnapshot.totalSpent)}{' '}
                  <span className="font-normal text-slate-400">/ {formatCurrency(data!.budgetSnapshot.totalBudgeted)}</span>
                </span>
              </div>
              <ProgressBar
                value={data!.budgetSnapshot.totalSpent}
                max={data!.budgetSnapshot.totalBudgeted}
                variant={data!.budgetSnapshot.totalSpent > data!.budgetSnapshot.totalBudgeted ? 'danger' : 'default'}
              />
              {data!.budgetSnapshot.overBudgetCategories.length > 0 && (
                <div className="mt-2 flex flex-wrap gap-1">
                  {data!.budgetSnapshot.overBudgetCategories.map((cat) => (
              ...
              <div className="flex items-center gap-3">
                <div className="h-2 flex-1 overflow-hidden rounded-full bg-slate-100">
                  <div
                    ...
                    style={{ width: `${(data!.actionPlan.completed / data!.actionPlan.total) * 100}%` }}
                  />
                </div>
                <span className="shrink-0 text-xs text-slate-500">
                  {data!.actionPlan.completed} / {data!.actionPlan.total} done
                </span>
              </div>
              {data!.actionPlan.nextActionTitle && (
                <div className="rounded-lg bg-slate-50 p-3">
                  <p className="text-xs font-medium text-slate-500">Next up</p>
                  <p className="mt-0.5 text-sm text-slate-800">{data!.actionPlan.nextActionTitle}</p>
                </div>
              )}
            </div>
          )}
      ...
      >
        {user?.tier === 'cfo' ? (
          <>
            ...
            <p className="mt-1 text-xs text-indigo-500">Ask ARIA anything about your finances. Coming soon.</p>
          </>
        ) : (
          <>
